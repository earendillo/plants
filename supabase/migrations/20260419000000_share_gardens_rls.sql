...
--
-- IMPORTANT: avoid RLS recursion.
-- `gardens` SELECT policy queries `garden_members`, so `garden_members` policies
-- must not query `gardens` directly (otherwise Postgres reports infinite recursion).
...
using (
  user_id = auth.uid()
  or public.is_garden_owner(garden_members.garden_id)
);
